# -*- coding: utf-8 -*-
"""
Created on Fri Nov 11 14:40:15 2016
...
class MarketData:
    def __init__(self):
        self.__recent_ticks__ = dict() # _recent_ticks_ is a dictionary of TickData object
...
class MarketDataSource:
    def __init__(self):
        self.event_tick=None # event_tick is a function
...
class Order:
    def __init__(self, timestamp, symbol, qty, is_buy, is_market_order, price=0):
        ...
        self.price = price
        self.is_buy = is_buy  #sell otherwise
...
# Base class for all the strategies        
class Strategy:
    def __init__(self):
...
import pandas as pd
# New strategy classes inherit the strategy class
class MeanRevertingStrategy(Strategy):
    ...
    # positions is a Position object which represents a change in position    
    def event_position(self, positions):
        if self.symbol in positions:
            position = positions[self.symbol]
            self.is_long = True if position.net > 0 else False
            self.is_short = True if position.net < 0 else False
    # the trade logic decision on every incoming tick data        
    # market_data is a MarketData object
    def event_tick(self, market_data):
    ...
    # The indicator that this strategy decide whether to buy or to sell 
...
class Backtester:
    def __init__(self, symbol, start_date, end_date, data_source="google"):
        ...
        self.positions = dict()  # A Position object
        self.current_prices = None  # A market data object
        self.rpnl, self.upnl = pd.DataFrame(), pd.DataFrame() # Realized and unrealized pnl
    ...
    # Print the order and set it to unfilled
    def evthandler_order(self, order):
        self.unfilled_orders.append(order)
        print(self.get_trade_date(), "Received order:", "BUY" if order.is_buy else "SELL",
    ...
    def print_position_status(self, symbol, prices): # prices is a MarketData object
        if symbol in self.positions:
            ...
            print(self.get_trade_date(), "Net:", position.net, "Value:",
                  position.position_value, "UPnL:", position.unrealized_pnl, 
    ...
    # start the backtest of a certain strategy class (strat)    
    def start_backtest(self,strat=MeanRevertingStrategy):
        self.strategy = strat(self.target_symbol) # Strategy 
